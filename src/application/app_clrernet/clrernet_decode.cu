...
#include "clrernet.hpp"
#include <common/cuda_tools.hpp>

namespace CLRerNet{

    const int NUM_LANE_ELEMENT = 7 + N_OFFSETS;    // _, score, start_y, start_x, theta, length, keepflag, (2*36)
    ...
    void nms_kernel_invoker(float* parray, float nms_threshold, int max_lanes, int input_width, cudaStream_t stream){

        auto grid  = CUDATools::grid_dims(max_lanes);
        auto block = CUDATools::block_dims(max_lanes);
        checkCudaKernel(nms_kernel<<<grid, block, 0, stream>>>(parray, max_lanes, input_width, nms_threshold));
    }
};