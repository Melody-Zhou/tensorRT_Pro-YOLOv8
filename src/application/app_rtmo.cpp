...
static void inference_and_performance(int deviceid, const string& engine_file, TRT::Mode mode, const string& model_name){
    ...
    auto engine = RTMO::create_infer(
        engine_file,                    // engine file
        deviceid,                       // gpu id
        0.25f,                          // confidence threshold
        0.45f,                          // nms threshold
        RTMO::NMSMethod::FastGPU,   // NMS method, fast GPU / CPU
        1024,                           // max objects
    ...
    auto files = iLogger::find_files("inference_rtmo", "*.jpg;*.jpeg;*.png;*.gif;*.tif");
    ...
    // warmup
    vector<shared_future<RTMO::BoxArray>> boxes_array;
    for(int i = 0; i < 10; ++i)
    ...
    INFO("%s[RTMO] average: %.2f ms / image, FPS: %.2f", engine_file.c_str(), inference_average_time, 1000 / inference_average_time);
    append_to_file("perf.result.log", iLogger::format("%s,RTMO,%s,%f", model_name.c_str(), mode_name, inference_average_time));

    string root = iLogger::format("%s_RTMO_%s_result", model_name.c_str(), mode_name);
    iLogger::rmtree(root);
    iLogger::mkdir(root);
...
static void test(TRT::Mode mode, const string& model){
    ...
    auto int8process = [=](int current, int count, const vector<string>& files, shared_ptr<TRT::Tensor>& tensor){
        ...
        for(int i = 0; i < files.size(); ++i){
            auto image = cv::imread(files[i]);
            RTMO::image_to_tensor(image, tensor, i);
        }
    };

    const char* name = model.c_str();
    INFO("===================== test RTMO %s %s ==================================", mode_name, name);
...
static void test_video(){
    ...
    auto engine = RTMO::create_infer(
        "rtmo-s_8xb32-600e_body7-640x640.FP32.trtmodel",   // engine file
        0,                              // gpu id
        0.25f,                          // confidence threshold
        0.45f,                          // nms threshold
        RTMO::NMSMethod::FastGPU,   // NMS method, fast GPU / CPU
...
static void test_single_image(){
    ...
    cv::Mat image = cv::imread("inference_rtmo/000000000785.jpg");
    if(image.empty()){
    ...
    INFO("Save to Result-rtmo.jpg, %d objects", boxes.size());
    cv::imwrite("Result-rtmo.jpg", image);
    engine.reset();
}

int app_rtmo(){
 
    test(TRT::Mode::FP32, "rtmo-s_8xb32-600e_body7-640x640");
    // test_single_image();
    // test_video();
    return 0;
}