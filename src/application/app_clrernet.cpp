#include "app_clrernet/clrernet.hpp"
...
static void inference_and_performance(int deviceid, const string& engine_file, TRT::Mode mode, const string& model_name){
    ...
    auto engine = CLRerNet::create_infer(
        ...
        4,                                      // nms topk
        270,                                    // cut height
        CLRerNet::NMSMethod::FastGPU,           // NMS method, fast GPU / CPU
        128,                                    // max lanes
    ...
    // warmup
    vector<shared_future<CLRerNet::LaneArray>> lanes_array;
    for(int i = 0; i < 10; ++i)
    ...
    INFO("%s[CLRerNet] average: %.2f ms / image, FPS: %.2f", engine_file.c_str(), inference_average_time, 1000 / inference_average_time);
    append_to_file("perf.result.log", iLogger::format("%s,CLRerNet,%s,%f", model_name.c_str(), mode_name, inference_average_time));

    string root = iLogger::format("%s_CLRerNet_%s_result", model_name.c_str(), mode_name);
    iLogger::rmtree(root);
    iLogger::mkdir(root);
...
static void test(TRT::Mode mode, const string& model){
    ...
    auto int8process = [=](int current, int count, const vector<string>& files, shared_ptr<TRT::Tensor>& tensor){
        ...
        for(int i = 0; i < files.size(); ++i){
            auto image = cv::imread(files[i]);
            CLRerNet::image_to_tensor(image, tensor, i, cut_height);
        }
    };
...
static void test_video(){
    ...
    auto engine = CLRerNet::create_infer(
        "clrernet.sim.FP16.trtmodel",           // engine file
        0,                                      // gpu id
        0.4f,                                   // confidence threshold
        50.0f,                                  // nms threshold
        4,                                      // nms topk
...
static void test_single_image(){
    ...
    INFO("Save to clrernet_result.jpg, %d lanes", lanes.size());
    cv::imwrite("clrernet_result.jpg", image);
    engine.reset();
}

int app_clrernet(){

    test(TRT::Mode::FP16, "clrernet.sim");
    // test_single_image();
    // test_video();
    return 0;
}