...
namespace LaneATT{
    ...
    void nms_kernel_invoker(
        float* parray, float nms_threshold, 
        int max_lanes, cudaStream_t stream
    );

    static float LaneIoU(const Lane& a, const Lane& b){
    ...
    static LaneArray cpu_nms(LaneArray& lanes, float threshold, int nms_topk){
    ...
    public:
        ...
        virtual bool startup(
            ...
            float nms_threshold, int nms_topk, NMSMethod nms_method,
            ...
            while(get_jobs_and_wait(fetch_jobs, max_batch_size)){
                ...
                for(int ibatch = 0; ibatch < infer_batch_size; ++ibatch){
                    ...
                    if(nms_method_ == NMSMethod::FastGPU){
                        nms_kernel_invoker(output_array_ptr, nms_threshold_, max_lanes_, stream_);
                    }
                }
                ...
                for(int ibatch = 0; ibatch < infer_batch_size; ++ibatch){
                    ...
                    if(nms_method_ == NMSMethod::CPU){
                        image_based_lanes = cpu_nms(image_based_lanes, nms_threshold_, nms_topk_);
    ...
    shared_ptr<Infer> create_infer(
        ...
        float nms_threshold, int nms_topk, NMSMethod nms_method, int max_lanes,
        bool use_multi_preprocess_stream
    ){
        shared_ptr<InferImpl> instance(new InferImpl());
        if(!instance->startup(
            engine_file, gpuid, confidence_threshold, nms_threshold,
            nms_topk, nms_method, max_lanes, use_multi_preprocess_stream)
        ){
            instance.reset();
        }
        return instance;
