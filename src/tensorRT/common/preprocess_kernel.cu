...
namespace CUDAKernel{
	...
	*/
	...
	__global__ void cut_resize_bilinear_and_normalize_kernel(
		...
		int dy      = position / dst_width;
		float src_x = (dx + 0.5f) * sx - 0.5f;
		float src_y = (dy + 0.5f) * sy - 0.5f + cut_height;
		float c0, c1, c2;
    ...
    void resize_normalize_image(
		...
		checkCudaKernel(resize_normalize_image_kernel << <grid, block, 0, stream >> > (
			src, src_line_size,
			src_width, src_height, dst,
			dst_width, dst_height, src_width/(float)resized_w, src_height/(float)dst_height, resized_w, norm, jobs
		));
	}
