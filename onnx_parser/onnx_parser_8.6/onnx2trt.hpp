...
public:
    ...
    virtual void registerLayer(nvinfer1::ILayer* layer, std::string const& basename, ::onnx::NodeProto const* node) = 0;

    //! Short form of register layer to use when the basename is the node's name.
    virtual void registerLayer(nvinfer1::ILayer* layer, ::onnx::NodeProto const& node) = 0;
    ...
    virtual void addUsedVCPluginLibrary(
        ::onnx::NodeProto const& node, char const* pluginName, char const* pluginLib)
        = 0;
