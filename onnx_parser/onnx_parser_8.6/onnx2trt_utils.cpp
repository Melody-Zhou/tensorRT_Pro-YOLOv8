...
bool convertDtype(int32_t onnx_dtype, nvinfer1::DataType* trt_dtype)
{
    ...
    case ::onnx::TensorProto::UINT8: *trt_dtype = nvinfer1::DataType::kUINT8; break;
    ...
    const ::onnx::TensorProto& onnxTensor, onnx2trt::ShapedWeights* weights, IImporterContext* ctx)
{
        ...
        || onnxDtype == ::onnx::TensorProto::INT8 || onnxDtype == ::onnx::TensorProto::BOOL)
    {
        ...
        else
        {
            switch (onnxDtype)
            {
            case ::onnx::TensorProto::INT32: dataPtr = (void*) (onnxTensor.int32_data().data()); break;
...
nvinfer1::ITensor* greaterLessOrEqual(IImporterContext* ctx, const ::onnx::NodeProto& node, nvinfer1::ITensor* inputA, nvinfer1::ITensor* inputB, bool greater)
{
...
nvinfer1::ITensor* iota(IImporterContext* ctx, ShapeTensor iotaDims, int32_t axis)
{
    ...
    auto* alpha = addConstantScalar(ctx, 0, ::onnx::TensorProto::INT32)->getOutput(0);
    auto* delta = addConstant(ctx, deltaVals, ::onnx::TensorProto::INT32, {1, {iotaDims.size()}})->getOutput(0);
    ...
    IImporterContext* ctx, const ::onnx::NodeProto& node, std::vector<TensorOrWeights>& inputs)
{
    ...
    auto kernel_weights = ShapedWeights::empty(::onnx::TensorProto::FLOAT);
    auto bias_weights = ShapedWeights::empty(::onnx::TensorProto::FLOAT);
...
NodeImportResult addScatterLayer(IImporterContext* ctx, ::onnx::NodeProto const& node,
...
float* convertFP16Data(void* weightValues, nvinfer1::Dims shape, IImporterContext* ctx)
{
    ...
    float* newWeights{static_cast<float*>(ctx->createTempWeights(::onnx::TensorProto::FLOAT, shape).values)};
...
Status processMetadata(::onnx::NodeProto const& node, nvinfer1::ILayer* layer)
{
