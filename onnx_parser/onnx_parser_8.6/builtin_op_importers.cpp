...
DEFINE_BUILTIN_OP_IMPORTER(BatchNormalization)
{
    ...
    auto const getValuesFP32 = [&](ShapedWeights const& w) -> float const* {
        return (w.type == ::onnx::TensorProto::FLOAT) ? static_cast<float*>(w.values)
    ...
    auto combinedScale = ctx->createTempWeights(::onnx::TensorProto::FLOAT, scale.shape);
    auto combinedBias = ctx->createTempWeights(::onnx::TensorProto::FLOAT, bias.shape);
    ...
    return scaleHelper(ctx, node, *tensorPtr, nvinfer1::ScaleMode::kCHANNEL, combinedBias, combinedScale,
        ShapedWeights::empty(::onnx::TensorProto::FLOAT), combinedBias.getName(), combinedScale.getName());
}
    ...
    if (ctx->getOpsetVersion() >=12)
    {
        ...
        if (attrs.count("value_floats"))
        {
            ...
            ShapedWeights convertedWeights
                = ctx->createTempWeights(::onnx::TensorProto::FLOAT, {1, {valueSize}});
        ...
        if (attrs.count("value_ints"))
        {
            ...
            int32_t valueSize = values.size();
            ShapedWeights convertedWeights
                = ctx->createTempWeights(::onnx::TensorProto::INT32, {1, {valueSize}});
            std::memcpy(convertedWeights.values, values.data(), convertedWeights.count() * sizeof(float));
            return {{convertedWeights}};
        }
    }
...
DEFINE_BUILTIN_OP_IMPORTER(ConvTranspose)
{
    ...
    {
        staticBiasWeights = ShapedWeights::empty(::onnx::TensorProto::FLOAT);
    }
    ...
    auto const emptyBiasWeights = ShapedWeights::empty(::onnx::TensorProto::FLOAT);
...
NodeImportResult QuantDequantLinearHelper(
    IImporterContext* ctx, ::onnx::NodeProto const& node, std::vector<TensorOrWeights>& inputs, bool isDQ, DataType datatype)
{
    CHECK(notInvalidType(inputs.at(0), {"UINT8"}));
...
nvinfer1::ITensor* concatenateRNNOutputs(IImporterContext* ctx, ::onnx::NodeProto const& node,
...
DEFINE_BUILTIN_OP_IMPORTER(If)
{
    ...
    ::onnx::GraphProto const& thenGraph = attrs.get<::onnx::GraphProto const&>("then_branch");
    ::onnx::GraphProto const& elseGraph = attrs.get<::onnx::GraphProto const&>("else_branch");
    ...
    if (cond.is_weights() && cond.weights().count() == 1)
    {
        ...
        ::onnx::GraphProto const& body = value == 1 ? thenGraph : elseGraph;
...
DEFINE_BUILTIN_OP_IMPORTER(IsInf)
{
    ...
    std::fill(scalarDims.d, scalarDims.d + scalarDims.nbDims, 1);
    auto& zeroTensor = *addConstantScalar(ctx, 0.F, ::onnx::TensorProto::FLOAT, scalarDims)->getOutput(0);
...
DEFINE_BUILTIN_OP_IMPORTER(LayerNormalization)
{
    ...
    auto biasType = input->getType() == nvinfer1::DataType::kFLOAT ? ::onnx::TensorProto::FLOAT
...
DEFINE_BUILTIN_OP_IMPORTER(Loop)
{
    ...
    ::onnx::GraphProto const& body = attrs.get<::onnx::GraphProto const&>("body");
    ...
    for (int32_t i = nbStateVars + NB_DISCARDED_OUTPUTS; i < nbOutputs; ++i)
    {
        ...
        else
        {
            trtScanOut->setInput(1,
                *addConstantScalar(ctx, DUMMY_SCAN_OUTPUT_LENGTH, ::onnx::TensorProto_DataType_INT32)
                     ->getOutput(0));
        }
...
DEFINE_BUILTIN_OP_IMPORTER(MeanVarianceNormalization)
{
    ...
    std::fill(scalarShape.d, scalarShape.d + scalarShape.nbDims, 1);
    auto* epsilonTensor = addConstantScalar(ctx, 1e-9f, ::onnx::TensorProto_DataType_FLOAT, scalarShape)->getOutput(0);
...
DEFINE_BUILTIN_OP_IMPORTER(Pad)
{
    ...
    if (onnxPadding.empty())
    {
        ...
        auto const zeros = addConstant(ctx, zerosVal, ::onnx::TensorProto::INT32, nvinfer1::Dims{1, {nbDims}})
                               ->getOutput(0);
        ...
        else if (value != 0.F)
        {
            ...
            case DataType::kINT8:
                fillValue = addConstant(
                    ctx, std::vector<float>{value}, ::onnx::TensorProto::FLOAT, nvinfer1::Dims{0, {0}})
                                ->getOutput(0);
                break;
            default:
                fillValue = addConstant(ctx, std::vector<int32_t>{static_cast<int32_t>(value)},
                    ::onnx::TensorProto::INT32, nvinfer1::Dims{0, {0}})
                                ->getOutput(0);
                break;
            }
...
NodeImportResult randomUniformHelper(IImporterContext* ctx, ::onnx::NodeProto const& node,
    ShapeTensor const& inputShape, OnnxAttrs const& attrs, DataType const& inputDType)
{
    ...
    if (attrs.count("dtype"))
    {
        ...
        switch (dtype)
        {
        case ::onnx::TensorProto::FLOAT: fillLayer->setOutputType(0, DataType::kFLOAT); break;
        case ::onnx::TensorProto::FLOAT16: fillLayer->setOutputType(0, DataType::kHALF); break;
...
NodeImportResult randomNormalHelper(IImporterContext* ctx, ::onnx::NodeProto const& node,
     ShapeTensor const& inputShape, OnnxAttrs const& attrs, DataType const& inputDType)
{
    ...
    if (ctx->getOpsetVersion() >= 9)
    {
        ...
        if (scales_input.is_weights())
        {
            ...
            ASSERT((scales_weights.count() == static_cast<size_t>(nbDims))
                ...
                ErrorCode::kUNSUPPORTED_NODE);
            ASSERT((scales_weights.type == ::onnx::TensorProto::FLOAT)
...
DEFINE_BUILTIN_OP_IMPORTER(Shrink)
{
    ...
    nvinfer1::ITensor* lambdTensor
        = addConstant(ctx, std::vector<float>{lambd}, ::onnx::TensorProto::FLOAT, {0, {1}})->getOutput(0);
    ...
    nvinfer1::ITensor* biasTensor
        = addConstant(ctx, std::vector<float>{bias}, ::onnx::TensorProto::FLOAT, {0, {1}})->getOutput(0);
    CHECK(broadcastTensors(ctx, biasTensor, x));

    nvinfer1::ITensor* zeroTensor
        = addConstant(ctx, std::vector<float>{0.}, ::onnx::TensorProto::FLOAT, {0, {1}})->getOutput(0);
